...
c.	Fit a random coefficients model with a fixed quadratic time term for each treatment group. Take the covariance matrix for the intercept and slope (linear time) term unstructured. 
...
\begin{aligned}
Y_{IMP} &= b_{0i} + b_{1i}\cdot X_{time_c}+ b_2\cdot X_{time_c}^2 + b_3\cdot X_{trt}\cdot X_{time_c} + b_4\cdot X_{trt}\cdot X_{time_c}^2 + e_{ij} \\
...
*R Users* : 
The formula you will be using in your `reml` function would look like this (remember to center your `time` variable):
